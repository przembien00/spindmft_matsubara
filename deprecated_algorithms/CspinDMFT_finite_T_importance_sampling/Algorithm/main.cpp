...
int main( const int argC, char* const argV[] ){ // arguments required for boost program options
  ...
  do
  {
    ...
    // ====== Effective Sample Size diagnostic (importance sampling efficiency) ======
    // Only meaningful for the non-uncoupled branch where Z is a single global weight per sample.
    if( !my_pspace.uncoupled_spins )
    {
      const RealType ess = ( my_rtdata.Z_sqsum > RealType{0.} )
                         ? ( my_partition_function * my_partition_function ) / my_rtdata.Z_sqsum
                         : RealType{0.};
      my_rtdata.ess_list.emplace_back( ess );
      my_rtdata.ess_relative_list.emplace_back( ess / static_cast<RealType>( my_rtdata.get_num_Samples() ) );
      print::print_R0( my_rank, "ESS = " + std::to_string(ess) + " / " + std::to_string(my_rtdata.get_num_Samples())
                              + " (ESS/M = " + std::to_string( my_rtdata.ess_relative_list.back() ) + ")\n" );
    }
