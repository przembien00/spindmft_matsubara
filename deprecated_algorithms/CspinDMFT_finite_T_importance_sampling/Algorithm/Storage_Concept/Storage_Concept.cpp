...
std::vector<std::vector<RealType>> site_fields_to_vmatrix( const std::vector<FieldVector>& fields )
{
    ...
    {
        vmatrix[i][0] = fields[i][0];
        vmatrix[i][1] = fields[i][1];
        vmatrix[i][2] = fields[i][2];
    }
    return vmatrix;
}

bool store_index_pair_list_attribute( const hid_t ID, const std::string& name, const ps::IndexPairList& pairs )
{
    hsize_t dims[1] = { pairs.size() };
    hid_t space_id = H5Screate_simple( 1, dims, nullptr );

    hsize_t pair_dims[1] = { 2 };
    hid_t pair_type = H5Tarray_create2( H5T_NATIVE_ULONG, 1, pair_dims );
    hid_t attr_id = H5Acreate2( ID, name.c_str(), pair_type, space_id, H5P_DEFAULT, H5P_DEFAULT );
    herr_t status = H5Awrite( attr_id, pair_type, pairs.data() );
...
void HDF5_Storage::store_main( const ps::ParameterSpace& pspace, rtd::RunTimeData& rtdata, CluCorrTen& CCT_Re, CluCorrTen& CCT_Im, const std::vector<FieldVector>& spin_expvals )
{
    if( !m_storing_permission ){ return; } // permission request
    ...
    store_index_pair_list_attribute( ps_group_id, "correlation_categories", pspace.correlation_categories );
    ...
    store_correlation_tensor_cluster( rtdata.sample_stds_Re, rtd_group_id, "Re_correlation_sample_stds", "Standard deviations of the real part of the imaginary-time correlations <S^alpha_i(tau)S^beta_j(0)>, stored according to the hierarchy i-j, alpha-beta, tau" );
    store_correlation_tensor_cluster( rtdata.sample_stds_Im, rtd_group_id, "Im_correlation_sample_stds", "Standard deviations of the imaginary part of the imaginary-time correlations <S^alpha_i(tau)S^beta_j(0)>, stored according to the hierarchy i-j, alpha-beta, tau" );
    hdf5r::store_2D_tensor<RealType>( rtd_group_id, "spin_expectation_values_stds", H5_REAL_TYPE, site_fields_to_vmatrix( rtdata.spin_expval_stds ), "Standard deviations of the thermal spin expectation values <S_i^alpha>, stored according to the hierarchy i, alpha" );
    ...
    // ...importance sampling effective sample size diagnostic (non-uncoupled branch)
    if( !rtdata.ess_list.empty() )
    {
        hdf5r::store_list( rtd_group_id, "ess_list", rtdata.ess_list );
        hdf5r::store_list( rtd_group_id, "ess_relative_list", rtdata.ess_relative_list );
    }
    ...
    store_correlation_tensor_cluster( CCT_Re, results_group_id, "Re_correlation", "Finite-temperature real part of imaginary-time correlations <S^alpha_i(tau)S^beta_j(0)>, stored according to the hierarchy i-j, alpha-beta, tau" );
    store_correlation_tensor_cluster( CCT_Im, results_group_id, "Im_correlation", "Finite-temperature imaginary part of imaginary-time correlations <S^alpha_i(tau)S^beta_j(0)>, stored according to the hierarchy i-j, alpha-beta, tau" );
    hdf5r::store_2D_tensor<RealType>( results_group_id, "spin_expectation_values", H5_REAL_TYPE, spin_expvals, "Thermal spin expectation values <S_i^alpha>, stored according to the hierarchy i, alpha" );
...
void HDF5_Storage::store_correlation_tensor_cluster( CluCorrTen& data, const hid_t data_group_id, const std::string dataset_name, const std::string dataset_info )
{
    ...
    {
        hdf5r::store_2D_tensor<RealType>(ct_data_group_id, std::to_string(sites[0])+"-"+std::to_string(sites[1]), H5_REAL_TYPE, CT);
    } );
