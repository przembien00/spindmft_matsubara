...
int main( const int argC, char* const argV[] ){ // arguments required for boost program options
  ...
  do
  {
    ...
    auto [my_meanfield_mean, my_meanfield_covariances] = my_pspace.mf_model->self_consistency( my_spin_correlations_Re, my_spin_expvals );
    ...
    while( remaining_samples > 0 ) // efficiency gain through cache coherence : the noise is sampled in sets beforehand 
    {
      ...
      {
        // ====== Read one full imaginary-time mean-field trajectory ======
        func::TimeTrajectory meanfield_trajectory{};
        meanfield_trajectory.reserve( my_pspace.num_TimePoints );
        for( size_t time = 0; time < my_pspace.num_TimePoints; ++time )
        {
          meanfield_trajectory.emplace_back( scheme.read() );
        }

        // ====== Propagate in imaginary time and sample thermal correlations ======
        auto [Z, forward_propagators, backward_propagators] = func::compute_propagators( meanfield_trajectory, my_meanfield_mean, my_pspace );
