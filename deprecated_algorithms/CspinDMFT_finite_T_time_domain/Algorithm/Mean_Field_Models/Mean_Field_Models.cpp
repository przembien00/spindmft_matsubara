...
std::vector<double> MeanFieldModel::read_configuration_from_file( const ps::ParameterSpace & pspace )
{
    ...
    // d) first-moment mean-field weights:
    std::vector<double> mf_expectation_weights_linearized{};
    hdf5r::import_ND_tensor_linearized( file_id, "expectation weights", mf_expectation_weights_linearized );
    mf_expectation_weights.resize( num_Spins, num_Spins );
    for( size_t row = 0; row < num_Spins; ++row )
    {
        auto start = mf_expectation_weights_linearized.begin() + row * num_Spins;
        auto end = start + num_Spins;
        std::copy( start, end, mf_expectation_weights.begin(row) );
    }

    // e) mean-field correlation weights:
    std::vector<double> Jsq_linearized{};
    hdf5r::import_ND_tensor_linearized( file_id, "correlation weights", Jsq_linearized );

    // f) correlation_categories:
...
void MeanFieldModel::hand_back_general_parameters( ps::ParameterSpace & pspace ) const
{
    ...
    pspace.J = J;
    pspace.mf_expectation_weights = mf_expectation_weights;
...
// =================== CLASS MULTI MODEL ==================
// ========================================================
void MultiModel::compute_Hamiltonian( SparseObservable& new_Hamiltonian, const VecVec&& Vs_of_t, const SiteFields& mean_fields, const char symmetry_type ) const
{
    new_Hamiltonian = H_CLUSTER;
    stda::for_n_each( [&new_Hamiltonian]( const auto& Sxi, const auto& Syi, const auto& Szi, const auto& Vi_of_t, const auto& mean_i )
    {
        auto total_field = Vi_of_t + mean_i;
        new_Hamiltonian += total_field[0]*Sxi + total_field[1]*Syi + total_field[2]*Szi;
    }, S_X_LIST.cbegin(), S_X_LIST.cend(), S_Y_LIST.cbegin(), S_Z_LIST.cbegin(), Vs_of_t.cbegin(), mean_fields.cbegin() );
...
std::tuple<SiteFields, mvgb::CovarianceMatrixBlocks> CorrelationReplicaModel::self_consistency( const CluCorrTen& env_spin_corr, const SiteFields& env_spin_expvals ) const 
{
    ...
    SiteFields rotated_env_spin_expvals( num_Spins, FieldVector{0.,0.,0.} );
    for( size_t k = 0; k < num_Spins; ++k )
    {
        rotated_env_spin_expvals[k] = D * env_spin_expvals[k];
    }

    SiteFields mf_mean( num_Spins, FieldVector{0.,0.,0.} );
    for( size_t i = 0; i < num_Spins; ++i )
    {
        for( size_t k = 0; k < num_Spins; ++k )
        {
            mf_mean[i] += mf_expectation_weights(i,k) * rotated_env_spin_expvals[k];
        }
    }
    ...
    mf_CCT.iterate( [&]( auto& CT, const IndexPair& ij )
    {
        ...
        env_spin_corr_rotated.iterate( [&]( CorrTen& SkSl_rot, const IndexPair& kl )
        {
            ...
            {
                Corr connected_corr = SkSl_rot_ab;
                const RealType connected_offset = rotated_env_spin_expvals[kl[0]][ab[0]] * rotated_env_spin_expvals[kl[1]][ab[1]];
                std::for_each( connected_corr.begin(), connected_corr.end(), [connected_offset]( RealType& value )
                {
                    value -= connected_offset;
                } );
                ViaVjb += J_mf_sq(ij[0],ij[1])(kl[0],kl[1]) * connected_corr;
            } );
        } );
    ...
    return std::make_tuple( std::move(mf_mean), std::move(mf_Cov) );
}
