...
class MeanFieldModel
{
 protected:
   ...
   IndexPairList correlation_categories{}; // correlations to be simulated
 ...
 public:
   ...
   virtual std::tuple<SiteFields, mvgb::CovarianceMatrixBlocks> self_consistency( const CluCorrTen& environment_spin_corr, const SiteFields& environment_spin_expvals ) const = 0;
...
class MultiModel : public MeanFieldModel
{
 public:
    ...
    // VIRTUAL AND NON-VIRTUAL METHODS AND OPERATORS
    virtual void compute_Hamiltonian( SparseObservable& new_Hamiltonian, const VecVec&& Vs_of_t, const SiteFields& mean_fields, const char symmetry_type ) const override;
};

// ===============================================================
// ========= DERIVED CLASS FOR CORRELATION REPLICA MODEL =========
// ===============================================================
/* Each mean-field is set up by out-of-cluster (environment) correlations that are related to in-cluster correlations via J_mf_sq */
class CorrelationReplicaModel : public MultiModel 
{
 private:
    // PRIVATE MODEL SPECIFIC PARAMETERS
    SymmMatrixOfMatrix J_mf_sq{}; // mean-field correlation weights, symmetric matrix of matrices (4D tensor)
 ...
 public:
    ...
    virtual std::tuple<SiteFields, mvgb::CovarianceMatrixBlocks> self_consistency( const CluCorrTen& environment_spin_corr, const SiteFields& environment_spin_expvals ) const override;
};
