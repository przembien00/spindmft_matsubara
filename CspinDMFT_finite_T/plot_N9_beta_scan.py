#!/usr/bin/env python3
"""
Plot imaginary-time spin correlations from N=9 uncoupled spins beta scan.
Creates three separate figures for site pairs (0,0), (0,1), (0,2),
each overlaying all 6 beta values (1, 2, 3, 5, 8, 12).

Correlation data: Re_correlation (real part of g^iso_{ij}(tau))
...
DATA = os.path.join(ROOT, "Data", "Uncoupled_N9_scan")
PLOTS = os.path.join(ROOT, "Plots", "Uncoupled_N9_beta_scan")
...
# 0-1: NN (nearest neighbor); 0-4: NNN (diagonal next-nearest neighbor)
PAIRS = [
    ("0-0", r"$g^{xx}_{0,0}(\tau)$"),
    ("0-1", r"$g^{xx}_{0,1}(\tau)$"),
    ("0-4", r"$g^{xx}_{0,4}(\tau)$"),
...
def data_path(beta):
    ...
    base = f"spinmodel=ISO__config=Square_2D_N=9_NN_J=0.0_uncoupled__beta={bstr}"
    for suffix in ["", "_ETV"]:
...
def load_correlation(path, pair_key):
    ...
    with h5py.File(path, "r") as f:
        beta = float(f["parameters"].attrs["beta"])
