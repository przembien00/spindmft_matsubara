    ...
    world_size( world_size )
{
    ...
    "set the inverse temperature and total imaginary-time interval"
    )(
...
void ParameterSpace::read_initial_correlations_from_file()
{
    ...
    auto import_correlation_component = [&]( const std::string& group_name, CluList& imported_correlations_linearized )
    {
        hid_t sub_group_id = H5Gopen( file_id, ("/results/" + group_name).c_str(), H5P_DEFAULT );
        if( sub_group_id < 0 )
        {
            error::IMPORT_FILE_NOT_FOUND( __PRETTY_FUNCTION__, import_filename + ":/results/" + group_name );
        }
        ...
        {
            std::string dataset_name = std::to_string(ij[0]) + "-" + std::to_string(ij[1]);
    ...
    import_correlation_component( "Re_correlation", imported_correlations_Re_linearized );
    import_correlation_component( "Im_correlation", imported_correlations_Im_linearized );

    hid_t results_group_id = H5Gopen( file_id, "/results", H5P_DEFAULT );
    if( results_group_id < 0 || H5Lexists( results_group_id, "spin_expectation_values", H5P_DEFAULT ) <= 0 )
    {
        error::IMPORT_FILE_NOT_FOUND( __PRETTY_FUNCTION__, import_filename + ":/results/spin_expectation_values" );
    }

    std::vector<RealType> imported_spin_expvals_linearized{};
    std::vector<hsize_t> spin_expval_dims{};
    hdf5r::import_ND_tensor_linearized( results_group_id, "spin_expectation_values", imported_spin_expvals_linearized, spin_expval_dims );
    if( spin_expval_dims.size() != 2 || spin_expval_dims[0] != num_Spins || spin_expval_dims[1] != 3 )
    {
        error::INIT_CORRELATIONS_MISMATCH( __PRETTY_FUNCTION__ );
    }
    imported_spin_expvals = std::vector<FieldVector>( num_Spins, FieldVector{0.,0.,0.} );
    for( size_t site = 0; site < num_Spins; ++site )
    {
        for( size_t alpha = 0; alpha < 3; ++alpha )
        {
            RealType value = imported_spin_expvals_linearized[3*site + alpha];
            if( std::isnan( value ) )
            {
                error::INIT_CORRELATIONS_CONTAIN_NANS( __PRETTY_FUNCTION__ );
            }
            imported_spin_expvals[site][alpha] = value;
        }
    }
