...
int main()
{
    ...
    Matrix mf_expectation_weights{{1.,1.},{1.,1.}};
    Matrix ones{{1.,1.},{1.,1.}};
    SymmMatrixOfMatrix J_mf_sq = blaze::declsym( MatrixOfMatrix{{ones,ones},{ones,ones}} );
    create_config_file( project_name, config_file, J, mf_expectation_weights, J_mf_sq );

    config_file = "Test_2";
    J = SymmMatrix{{0.,1.,0.},{1.,0.,0.},{0.,0.,0.}};
    mf_expectation_weights = Matrix{{0.,0.,1.},{0.,0.,1.},{0.,0.,1.}};
