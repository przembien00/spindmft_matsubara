...
int main( const int argC, char* const argV[] ){ // arguments required for boost program options
  ...
  CluCorrTen my_spin_correlations = init::generate_initial_environment_spin_correlations( my_pspace );
  ...
  do
  {
    ...
    auto my_meanfield_covariances = my_pspace.mf_model->self_consistency( my_spin_correlations );
    ...
    my_rtdata.compute_and_process_sample_stds( my_new_spin_correlations );
    ...
    my_rtdata.compute_iteration_error( my_new_spin_correlations, my_spin_correlations );
    my_spin_correlations = std::move( my_new_spin_correlations );
  ...
  my_data_storage.store_main( my_pspace, my_rtdata, my_spin_correlations );
