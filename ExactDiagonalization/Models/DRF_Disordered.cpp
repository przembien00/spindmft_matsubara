...
// CONSTRUCTOR
DRF_Disordered_Model::DRF_Disordered_Model( const ps::ParameterSpace& pspace ):
    Model( "DRF_Disordered", 'a', 'A', pspace, true )
{} 

// generate string with compact info, e.g., for filenames
std::string DRF_Disordered_Model::compact_info( const uint num_Digits ) const
{
    std::string info = "DRF";
    return info;
}
...
std::string DRF_Disordered_Model::init_state_info( const uint num_Digits ) const
{
    return "Disordered";
}

// return vector of Parameters
std::vector<Parameter> DRF_Disordered_Model::return_params() const
{
...
void DRF_Disordered_Model::compute_Hamiltonian()
{
    ...
    { 
        for(uint spin_j = spin_i+1; spin_j < num_Spins; spin_j++) // spin_i is always in front of spin_j
        {
            if( J( spin_i, spin_j ) != RealType{0.} ) // e.g. for nearest-neighbor interactions
            {
                ...
                H += RealType{0.125} * J( spin_i, spin_j ) * ( - Vx + Vy + Vz ); 
                /* the factor 0.125 is due to the conversion sigma->S (0.25) and the model (-0.5 SxSx -0.5 SySy + SzSz)
                the rotation is diagonal, so only three terms are occurring here 
                the relative minus sign between Vx and Vy is due to using the real version of sigma_y*/
            }
        }
    }
}
...
void DRF_Disordered_Model::transform_to_diagonal_basis()
{
#ifndef SPARSE
    ...
    // construct Sx1 for the central spin
    RealObservable Sx1{}; 
    init::build_linear_spin_operator_hermit( Sx1, num_Spins, 0, SIGMA_X_REAL );
    RealObservable Sx2{}; 
    init::build_linear_spin_operator_hermit( Sx2, num_Spins, 1, SIGMA_X_REAL );
    RealObservable Sx3{}; 
    init::build_linear_spin_operator_hermit( Sx3, num_Spins, 2, SIGMA_X_REAL );

    // make them sparse
    SparseRealObservable Sx1_SPARSE{ Sx1 };
    func::truncate_sparse( Sx1_SPARSE, RealType{0.0001} );
    SparseRealObservable Sx2_SPARSE{ Sx2 };
    func::truncate_sparse( Sx2_SPARSE, RealType{0.0001} );
    SparseRealObservable Sx3_SPARSE{ Sx3 };
    func::truncate_sparse( Sx3_SPARSE, RealType{0.0001} );

    // transform to the diagonal basis
    Sx1_D = blaze::declsym(  K_T_SPARSE * Sx1_SPARSE * K_SPARSE );
    Sx2_D = blaze::declsym(  K_T_SPARSE * Sx2_SPARSE * K_SPARSE );
    Sx3_D = blaze::declsym(  K_T_SPARSE * Sx3_SPARSE * K_SPARSE );
    func::write_to_diagonal( H_D, omega );
...
void DRF_Disordered_Model::compute_means_and_autocorrelations_at( const RealType& time )
{
    // PAIRCORRELATIONS:
    // compute <Sx1(t)Sx2(0)Sx3(0)>
    ...
    Operator L = Uplus  * Observable{Sx1_D};
    Operator R = Uminus * Observable{Sx2_D} * Observable{Sx1_D};
    RealType SxSxSx = rho * std::real( blaze::trace( L * R ) ); // efficient trace
    m_correlations.get_xx().emplace_back( 0.125 * SxSxSx );
