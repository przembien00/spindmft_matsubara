#!/usr/bin/env bash
# 6 beta values x 7 sample counts x 2 cases = 84 jobs.
# Submit from either the repository root or spinDMFT_Keldysh/ with:
#   sbatch spinDMFT_Keldysh/slurm_beta_sample_scan.sh
#   sbatch slurm_beta_sample_scan.sh
# To submit only a subset, for example tasks 0 through 41:
#   sbatch --array=0-41 slurm_beta_sample_scan.sh
